...
# Variables
$DownloadUrl   = "https://s3.amazonaws.com/assets-cp/assets/Agent_Uninstall.zip"
...
Try {
    ...
    Write-Host "Downloading Automate Uninstall ZIP..."
    ...
    # Run Agent_Uninstall.exe to generate uninstall.exe
    $AgentUninstall = Join-Path $ExtractPath "Agent_Uninstall.exe"
    if (-not (Test-Path $AgentUninstall)) {
        Throw "Agent_Uninstall.exe not found in $ExtractPath"
    }

    Write-Host "Running Agent_Uninstall.exe..."
    Start-Process -FilePath $AgentUninstall -Wait -NoNewWindow

    # Run uninstall.exe silently
    $UninstallEXE = Join-Path $ExtractPath "uninstall.exe"
    if (-not (Test-Path $UninstallEXE)) {
        Throw "uninstall.exe not created by Agent_Uninstall.exe"
    }
